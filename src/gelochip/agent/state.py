...
class GelochipAgentState(TypedDict):
    ...
    # PySpice netlist generated by circuit_designer for pre-layout validation
    pyspice_code: Optional[str]

    # Result of PySpice/ngspice simulation (stdout, exit_code, success, error)
    pyspice_result: Optional[dict[str, Any]]
    ...
    # Structured feedback from corrector back to the failed node
    # Contains error analysis + what was tried, so the node avoids the same mistakes
    corrector_feedback: Optional[str]

    # Which node triggered the corrector ("circuit_designer" | "layout_generator" | ...)
